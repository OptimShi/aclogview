...
namespace aclogview
{
    public partial class FindOpcodeInFilesForm : Form
    {
        ...
        private string logFileName { get { return "D:\\Source\\Creature-PlayScript-" + dt.ToString("yyyy-MM-dd") + ".csv"; } }

        private void ResetLogFile()
        {
            using (StreamWriter theFile = new StreamWriter(logFileName, false))
                theFile.WriteLine("wcid,name,PlayScript,Mod");
        }
        ...
        private void ProcessFile(string fileName)
        {
            ...
            Dictionary<uint, CM_Physics.CreateObject> CreatureList = new Dictionary<uint, CM_Physics.CreateObject>();
                ...
                try
                {
                    ...
                    switch (opcode)
                    {
                        ...
                        case PacketOpcode.Evt_Physics__CreateObject_ID:
                            ...
                            if(message.wdesc._type == ITEM_TYPE.TYPE_CREATURE && createWcid != 1) {
                                CreatureList[objectId] = message;
                            }
                            break;
                        case PacketOpcode.Evt_Physics__DeleteObject_ID:
                            ...
                            if (CreatureList.ContainsKey(deleteObjectId))
                                CreatureList.Remove(deleteObjectId);
                            break;
                        case PacketOpcode.Evt_Physics__PlayScriptType_ID:
                            var psMessage = CM_Physics.PlayScriptType.read(messageDataReader);
                            var psObjectId = psMessage.object_id;
                            if (CreatureList.ContainsKey(psObjectId))
                            {
                                string name = CreatureList[psObjectId].wdesc._name.m_buffer;
                                uint wcid = CreatureList[psObjectId].wdesc._wcid;

                                var type = psMessage.script_type;
                                if(type != PScriptType.PS_Explode
                                    && type != PScriptType.PS_Launch
                                    && type != PScriptType.PS_Create
                                    && type != PScriptType.PS_DirtyFightingDamageOverTime
                                    && type != PScriptType.PS_DirtyFightingDefenseDebuff
                                    && type != PScriptType.PS_DirtyFightingAttackDebuff
                                    && type != PScriptType.PS_DirtyFightingHealDebuff
                                    && (type < PScriptType.PS_SplatterLowLeftBack || type > PScriptType.PS_SparkUpRightFront)
                                    && (type.ToString().IndexOf("Down") == -1)
                                    )
                                {
                                    var ps_string = $"{wcid},\"{name}\",{type},{psMessage.mod}";
                                    PS_List.Add(ps_string);
                                }
                            }
                            break;
                    }
